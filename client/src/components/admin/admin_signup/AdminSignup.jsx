...
import { adminSignup } from "../../../actions/adminAuth";


const AdminSignup = () => {

    const [ signupData, setSignupData ] = useState({ firstName: '', lastName:'', email: '', mobileNo: '', address: '', sec_key:'', adharImage: '', contractPaperImage: '', password: '',confirmPassword: '' });
    ...
    const handleGetOtp = (e) => {
        ...
        axios.post('http://localhost:5000/admin/generateOtpForEmail', emailVerificationData)
         .then(res =>{
    ...
    const handleOtpSubmit = (e) => {
        ...
        axios.post('http://localhost:5000/admin/verifyOtpForEmail', emailVerificationData)
         .then(res =>{ 
    ...
    const handleSubmit = (e) => {
        e.preventDefault();          
        dispatch(adminSignup(signupData, navigate));
    }    
     

    const handleClear = () => {
        setSignupData({ firstName: '', lastName:'', email: '', mobileNo: '', address: '', sec_key:'', adharImage: '', contractPaperImage: '', sec_key:'', password: '', confirmPassword: '' });
    ...
    return (
        <>
            <form className="flex-column border border-dark" id="register" >            
                <h2 className="d-flex fw-bold mt-2" id="regtitle">Admin Signup</h2>
                ...
                <div className="col-12 mt-3 ms-2" id="fullwidthinput">
                <input type="text" className="form-control form-control-lg" placeholder="Full Address" aria-label="full address" value={signupData.address} onChange={(e) => setSignupData({ ...signupData, address: e.target.value })} />               
                </div> 

                <div className="col-12 mt-3 ms-2" id="fullwidthinput">
                <input type="text" className="form-control form-control-lg" placeholder="Secreate Key" aria-label="Secreate Key" value={signupData.sec_key} onChange={(e) => setSignupData({ ...signupData, sec_key: e.target.value })} />               
                </div> 

                <div className="row mt-1 ms-2" id="fullwidthinput">
                    <div className="col-6">
                        <label for="file_input1" className="col-form-label fw-bold">Adhar card image :</label> <br/>
                        <FileBase type="file" multiple={false} onDone={ ({ base64 }) => setSignupData({ ...signupData, adharImage: base64 }) } id='file_input1' /> <br />
                    </div>

                    <div className="col-6">
                        <label for="file_input2" className="col-form-label fw-bold">contract paper image :</label> <br/>
                        <FileBase type="file" multiple={false} onDone={ ({ base64 }) => setSignupData({ ...signupData, contractPaperImage: base64 }) } id='file_input2' /> <br />
                    </div>
                </div>                             
                ...
                <div className="col-12 d-flex justify-content-end mb-2">
                  <a className="me-3 fw-bold" href="/admin/login">Already have an account.Click here to Login</a>
                </div>

                <div className="col-12 d-flex justify-content-end mb-2">
                  <a className="me-3 fw-bold" href="/user/signup">User Signup ?</a>
                </div>    
            </form>
        </>
    )
