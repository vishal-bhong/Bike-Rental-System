...
export const signupAdmin = async (req, res) => {
    const { firstName, lastName, email, mobileNo, address, sec_key, adharImage, contractPaperImage, password, confirmPassword } = req.body;
    try {  
        ...
        const validate_sec_key = await sec_Keys.findOne({ sec_key });
        
        if(!validate_sec_key) return res.status(400).json({ message: "Invalid Security key" });
        
        const existing_sec_key = await Admin.findOne({ sec_key });
        
        if(existing_sec_key) return res.status(401).json({ message: "Security key already used" });
